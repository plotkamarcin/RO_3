...
public class houghDemo extends JApplet {
	...
	String fileNames[] = { "countdark.png", "test.png","test2.png","count1.png"};
	...
	public void init() {
		...
		circles = new ArrayList<circleHough>();
		
		tracker = new MediaTracker(this);
		for(int i = 0; i < fileNames.length; i++) {
			image[i] = getImage(this.getCodeBase(),fileNames[i]);
			image[i] = image[i].getScaledInstance(430, 430, Image.SCALE_SMOOTH);
		...
		linesSlider = new JSlider(JSlider.HORIZONTAL, 1, 501, lines);
		linesSlider.addChangeListener(new linesListener());
		linesSlider.setMajorTickSpacing(50);
   ...
   private void calculateCirclesAmount(){
	   ArrayList<int[]> results= new ArrayList<int[]>();
	   int amount =0;
	   for (circleHough c : circles){
		   results.add(c.results);
	   }
	   Integer[] vals =new Integer[results.get(0).length/3];
	   for(int i=0;i<results.get(0).length/3;i++){
		   vals[i]=(int)Math.pow(Math.pow(results.get(0)[i*3+1],2.0)+Math.pow(results.get(0)[i*3+2],2.0),0.5);
	   }
	   Set<Integer> uniqKeys = new TreeSet<Integer>();
	   uniqKeys.addAll(Arrays.asList(vals));
       
	   System.out.println("found " + uniqKeys.size() + " circles");
   }
}